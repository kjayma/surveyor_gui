...
//      if (!Modernizr.input.required) {
  ...
  $(document).on('click', '#add_question_centered, #add_question_off_centered', function() {
...
//      $('#question_answers_attributes_0_text').val('String');
  ...
  $.fn.show_answers = function (){
    if ($(this).val()
        in {
        'Multiple Choice (only one answer)':1,
...
//  });
  ...
  function show_dependency_value(data, target) {
    ...
    else if (/dropdown list/.test(question_type)) {
      target.find('.dependency_pick_multiple_choice').show();
    }
    else if (/slider/.test(question_type)) {
      target.find('.dependency_pick_multiple_choice').show();
    }
