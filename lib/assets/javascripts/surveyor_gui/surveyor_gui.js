...
//    );
  ...
  function render_question_group (){
    ...
    if (question_group_id != undefined)  {
...
//      function(data){
    window.location.href = "/question_groups"+question_group_clause
//        here.html(data);
//        here.check_dynamic_source();
//        here.sortable_answers();
//        update_uislider();
  ...
  $.fn.render_group_inline_fields_partial = function (add_row){
    ...
    $.get(
      "/question_group/render_group_inline_partial?id="
        + $(this).closest('.questions').find('input#question_group_id').first().val() + option,
      function(data){
        if (add_row == true) {
...
*/
  ...
  $('select[id^="question_group_questions_attributes_"][id$="question_type_id"]').on('change', function(){
    $(this).show_group_answers();
  });
