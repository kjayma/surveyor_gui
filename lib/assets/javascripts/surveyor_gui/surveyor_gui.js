    ...
    $.colorbox({
      ...
      onClosed:function(){
          ...
          if (submitted==="true") {
              ...
              {},
              function(response) {
                $(".column3").filter(":first").replaceWith(response);
                refresh_stars();
      ...
      href: new_question_url+'?survey_section_id='+survey_section_id,
      opacity:.3}
    );
  });
...
//      $('#question_answers_attributes_0_text').val('String');
  ...
  $.fn.render_picks_partial = function (){
    here = $(this)
    $.get(
      "/question/render_picks_partial?id=" + $(this).closest('.questions').find('input#question_id').val(),
      function(data){
        here.html(data);
  ...
  $.fn.render_picks_partial = function (add_row){
    here = $(this);
    if (add_row === true) {
    ...
    $.get(
      "/question/render_picks_partial?id=" + $(this).closest('.questions').find('input#question_id').val() + option,
      function(data){
        here.html(data);
        here.check_dynamic_source();
        here.sortable_answers();
  ...
  $('#answer_choice').on('click', '.add_answer img', function () {
    $(this).closest('#answer_choice').render_picks_partial(true);
  });
...
*/
  ...
  $('input#dependency_submit').on('click', update_logic).trigger('submit');
  ...
  $.fn.render_dependency_conditions_partial = function (add_row){
    if ($(this).length > 0) {
      ...
      $.get(
        "/dependency/render_dependency_conditions_partial?id=" + $(this).data('question_id') + option,
        function(data){
          if (add_row === true) {
          ...
          $('select[id^="question_dependency_attributes_dependency_conditions_attributes"][id$="question_id"]').each( function() {
            ...
            $(this).refresh_dependency_answers();
          });
          //$('input[id$="rule_key_temp"]').update_logic();
...
//    $(this).determine_dependency_question_type();
//  });
  $('.dependency_editor').on('change', 'select[id^="question_dependency_attributes_dependency_conditions_attributes"][id$="question_id"]', function(event) {
  ...
  function update_logic() {
      // Makes sure the last rule key is a greater value than the preceding keys.
      // Looks at the second to last rule_key (array comes sorted) to make determination.  E.g., if the existing rule is
      // "A or B or C or D", will find "C", calculate the next letter, and make the last rule key "D".  Seems redundant,
      // but works for when we are adding a new row that comes with a null rule key.  In this case, will find "D",
      // and set the final (null) rule key to "E".  Takes into account a rule of "Z", which is a special case.
      // Surveyor uses "Z" to compare a value to a count of answers, and is reserved.  Shouldn't look at it when finding the next
      // available rule key.
      var rule_keys = $('input[id$="rule_key_temp"]');
      if (rule_keys.length > 1) {

        var last_rule = rule_keys[rule_keys.length-1];

        var rule_keys_excluding_z = $.map(rule_keys,function(a){return a.value==="Z" ? null : a.value}).sort()
        var max_key_value = rule_keys_excluding_z[rule_keys.length-2];

        var char_no = max_key_value.charCodeAt() + 1;

        var last_key = String.fromCharCode(char_no);
      }
      else
      {
        var last_key = 'A';
      ...
      rule_keys.each(function(index) {
        if (index===0) {
          ...
          rule = rule +' '+$(this).parents('tr').prev('tr').find('select').val()+' '+ $(this).val()+' ';
        }
      });
  ...
  function show_dependency_value(data, target) {
    ...
    data = data.split(',');
    var pick = data[0];
    var question_type = data[1].toLowerCase();
...
//      alert($(link).parent().prev().find('select').last().attr('id'));
  ...
  $.fn.refresh_dependency_answers = function() {
    $(this).closest('tr').find('[id^="question_dependency_attributes_dependency_conditions_attributes"][id$="answer_id"]')
      .load(
