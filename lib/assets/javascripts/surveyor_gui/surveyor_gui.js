...
//      $('#question_answers_attributes_0_text').val('String');
  ...
  $.fn.render_grid_fields_partial = function (){
    here = $(this);
    $.get(
      "/question/render_grid_partial?id=" + $(this).closest('.questions').find('input#question_id').val(),
      function(data){
        here.html(data);
        here.check_dynamic_source();
        here.sortable_answers();
  ...
  $.fn.show_answers = function (){
    ...
    else if ($(this).val() in {'grid_one':1, 'grid_any':1}){      
      $('#question_answers_attributes_0_text').val($('#original_choice').data('original_choice'));   
      $('#sortable_answers').render_grid_fields_partial();
      $('#answer_choice').show();
    }
    else
    {
...
//  });
  ...
  function show_dependency_value(data, target) {
    ...
    if (/anyone/.test(question_type)) {
      target.find('.dependency_pick_multiple_choice').show();
    }
    else if (/dropdown/.test(question_type)) {
      target.find('.dependency_pick_multiple_choice').show();
    }
