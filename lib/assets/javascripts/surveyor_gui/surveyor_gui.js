...
//      $('#question_answers_attributes_0_text').val('String');
  ...
  $.fn.render_group_inline_fields_partial = function (add_row){
    ...
    $.get(
      "/question/render_group_inline_partial?id="
        + $(this).closest('.questions').find('input#question_id').val() + option,
      function(data){
        if (add_row == true) {
  ...
  $('#answer_choice').on('click','.add_group_inline_question', function() {
    $('#sortable_answers').render_group_inline_fields_partial(true);
  });
