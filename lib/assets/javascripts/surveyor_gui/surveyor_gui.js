...
//      $('#question_answers_attributes_0_text').val('String');
  ...
  function render_question (){
    ...
    window.location.href = "/questions"+question_clause
//    $.get(
//      "/questions"+question_clause,
//      function(data){
//        here.html(data);
//        here.check_dynamic_source();
//        here.sortable_answers();
...
//    );
  ...
  function render_question_group (){
    ...
    var text = $('textarea[id="question_text"]').val();
    var display_order = $('input[id$="display_order"]').first().val();
    var question_group_id = $('.questions').find('input#question_question_group_id').val()
...
//    $.get(
//      "/question_groups"+question_group_clause,
//      function(data){
    window.location.href = "/question_groups"+question_group_clause+"?survey_section_id="+survey_section_id+"&text="+text+
...
//    );
  ...
  $.fn.render_group_inline_fields_partial = function (add_row){
    ...
    var display_order = (this).closest('.questions').find('input[id$="display_order"]').first().val();
    ...
    $.get(
      ...
      function(data){
        if (add_row == true) {
          ...
          $('.group_inline_question').last().after(data);
        }
        else {
...
*/
  $('input[id^="question_question_type_"],input[id^="question_group_question_type_"]').on('change', function(event){
    if ($(this).val() != "group_inline" && $('.edit_question_group').length > 0){
      render_question();
    }
    else {
      $(this).show_answers();
  ...
  $('input:checked[id^="question_question_type_"]').each(function(link){
    $(link).show_number_questions();
    $(link).show_answers();
  });
...
//      };
  ...
  function show_dependency_value(data, target) {
    ...
    else if (question_type=="inline_group") {
      target.find('.dependency_pick_multiple_choice').show();
    }
